//Gains access to the classes, now we will create all of the functions
#include "SDRNN.h" 

//Random number generator to initialize weights with random values
double frand(){ 
...
Perceptron::Perceptron(int inputs, double bias){
	this->bias = bias;
	weights.resize(inputs+1); //sizing the weights
	generate(weights.begin(),weights.end(),frand);// Initalizing weights with random numbers
}

// Run the perceptron. x is a vector with the input values.
double Perceptron::run(vector<double> x){
	x.push_back(bias); //add the bias to the input values
	double sum = inner_product(x.begin(),x.end(),weights.begin(),(double)0.0);// calculating weighted sum
	return sigmoid(sum);//running weighted sum through the sigmoid function
}

// Set the weights. w_init is a vector with the weights.
void Perceptron::set_weights(vector<double> w_init){
	weights = w_init; //sets the weights to that of the parameter
}
...
vector<double> MultiLayerPerceptron::run(vector<double> x) {
    ...
    for (int i = 1; i < network.size(); i++) //iterates layers
        for (int j = 0; j < layers[i]; j++)//iterates neurons in each layer
            values[i][j] = network[i][j].run(values[i-1]);//running the current neuron by feeding it the values in the previous layer
    return values.back();
}
